...
def read_file(path, is_train=False):
    ...
    with open(path, "r") as f:
        ...
        for lines in f:
            ...
            if is_train:
                session_id_in_doc, custom_id_in_doc, is_assistance_in_doc = line[0], line[1], line[2]
            else:
                session_id_in_doc, custom_id_in_doc, is_assistance_in_doc = line[2], line[1], line[3]
            ...
            if session_id != session_id_in_doc and session_id != "":
                ...
                if is_assistance != is_assistance_in_doc and is_assistance != "":
                    content = filter_content(content)
                    is_assistance = "custom" if str(is_assistance) == "0" else "assistance"
...
def generate_data(filepath, save=True, to_file=None, pair=False):
    """
    将 read_file 的结果进行转化, 问答pair, 或者将一次绘画内容合并到一起
    :param filepath: 原始数据路径
    :param save: 是否保存
    :param to_file: 保存的文件名, 会根据文件名判断是否为训练集
    :param pair: 是否生成问答pair 的结果
